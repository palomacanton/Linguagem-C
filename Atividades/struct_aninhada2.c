	...
	int main() { 
		setlocale(LC_ALL,"portuguese"); //configuração de localização
		aluno.codigo = 0;
		strcpy(aluno.nome, "NULL"); // serve para atribuir "NULL" para a variével "nome", digamos inicializar a variável.
		aluno.datNasc.dia = 0;
		aluno.datNasc.mes = 0;
		aluno.datNasc.ano = 0;
		...
		printf(" \n A data de nascimento do aluno é: %d / %d / %d ", aluno.datNasc.dia, aluno.datNasc.mes, aluno.datNasc.ano);
		printf(" \n \n");
		system("pause"); //dependendo do sistema, serve para esperar o usuário digitar algo antes de continuar
		return(0);
	}												
